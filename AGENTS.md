...
> 完整版与维护说明见 `docs/development-principles.md`（2026-08-23 用户提供，DEV-001 落地）。
...
**执行约定**：本段随每次会话自动注入；任务执行、代码审查、验收证据均须对照上列原则逐条自检。开发原则 1 违反（无事实依据写成闭环）即流程违规；编程要求 4 违反（一个 commit 多个问题）即提交违规，由预提交钩子与 Review 把关。
