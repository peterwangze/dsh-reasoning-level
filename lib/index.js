...
/** discovery 探测独立短超时：网关对非法 effort 挂起时不阻塞候选组装。 */
...
export function apply(ctx) {
  ...
  if (webServer !== undefined) {
    ...
    // ── MAINT-014：必然失败探测（discovery）──────────────────────────────────────
    /** 从网关拒绝消息中解析模型真实支持的思考等级列表。
     *  已知模式：
     *    "Valid values: ['low','medium','high']"
     *    "reasoning effort \"xxx\" is not supported. Available: low, medium, high"
     *  返回 string[] | undefined。 */
    function parseLevelsFromMessage(message) {
      if (typeof message !== 'string' || message === '') return undefined
      // 模式 A: Valid values: ['low','medium','high']（含 variant Valid values: low, medium, high）
      let match = message.match(/[Vv]alid values:\s*\[?([^\]]+?)\]?(?:\s|\.|$)/)
      if (match) {
        const levels = match[1].split(',').map((s) => s.trim().replace(/['"]/g, '')).filter((s) => s.length > 0)
        if (levels.length > 0) return levels
      }
      // 模式 B: Available: low, medium, high  /  Supported: low, medium, high
      match = message.match(/(?:[Aa]vailable|[Ss]upported):\s*(.+?)(?:\.|$)/)
      if (match) {
        const levels = match[1].split(',').map((s) => s.trim().replace(/['"]/g, '')).filter((s) => s.length > 0)
        if (levels.length > 0) return levels
      }
      // 模式 C: 直接从副词片段提取被引号包裹的等级 token
      match = message.match(/'([a-zA-Z]\w*(?:[-+_][a-zA-Z]\w*)*)'/g)
      if (match) {
        const levels = [...new Set(match.map((m) => m.replace(/'/g, '')))].filter((s) => s !== '__dsh_discovery__')
        if (levels.length > 0) return levels
      }
      return undefined
    }

    /** 必然失败探测——发送一个无法被任何模型接受的 effort 值以触发网关拒绝，
     *  从失败响应中解析模型真实支持的思考等级列表。
     *  使用独立短超时（DISCOVERY_TIMEOUT_MS），超时后返回 undefined
     *  （回退候选，绝不阻塞/失败整个探测）。
     *  返回 string[] | undefined；解析失败或请求异常时回退（不引起探测整体失败）。 */
    async function discoverLevelsFromRejection(provider, model) {
      try {
        const result = await probeLevelOnce(provider, model, '__dsh_discovery__', DISCOVERY_TIMEOUT_MS)
        if (!result.ok && typeof result.message === 'string') {
          const levels = parseLevelsFromMessage(result.message)
     ...
     * 探测一个模型的全部候选等级（并发 3）。
     *
     * MAINT-014 增强：候选来源从"fallback 链"改为"并集"——
     *   目录声明 ∪ 配置声明 ∪ discovery 解析列表 ∪ 生成集。
     *   discovery 发现级（非标准档）不过 LEVELS 过滤。
     *
    ...
    async function probeModelLevels(provider, model, levelsFilter) {
      ...
      // 并行获取各候选来源：目录声明、配置声明、discovery（必然失败探测）
      const [resolveInfo, declared, discoveryLevels] = await Promise.all([
        (async () => {
          try {
            const info = await llm.resolveModelInfo(provider, model)
            if (info !== undefined && info.reasoning !== undefined && Array.isArray(info.reasoning.efforts)) {
              return info.reasoning.efforts.map((entry) => entry.id)
              // LEVELS 过滤已移除（MAINT-014）：非标准档也能进候选，由实测判定
            }
            return []
          } catch (error) { return [] }
        })(),
        Promise.resolve(declaredEffortsOf(provider, model)),
        discoverLevelsFromRejection(provider, model),
      ])
      // 并集组装（去重保持顺序）：目录声明（LEVELS 粒度）→ 配置声明 → discovery（不过 LEVELS）
      // GENERATED_LEVELS 仅在候选仍为空时作为 fallback（原 fallback 链语义不变）。
      const seen = new Set()
      const candidates = []
      function push(level) {
        if (!seen.has(level)) { seen.add(level); candidates.push(level) }
      }
      // 目录声明：不过 LEVELS 过滤（MAINT-014——非标准档也能进候选，由实测判定）
      for (const level of resolveInfo) push(level)
      // 配置声明（llm-pi-ai 能力声明，已按 LEVELS 过滤）
      if (declared !== undefined) { for (const level of supportedLevels(declared)) push(level) }
      // discovery 发现级：不过 LEVELS 过滤（qwen 类模型的非标准档是本任务目标）
      if (discoveryLevels !== undefined) { for (const level of discoveryLevels) push(level) }
      // fallback：无目录/配置/discovery 时使用生成集
      if (candidates.length === 0) { for (const level of GENERATED_LEVELS) push(level) }
      if (Array.isArray(levelsFilter) && levelsFilter.length > 0) {
        // levelsFilter 作用于最终候选集（含 discovery 级）：客户端显式筛选应被尊重。
        // 若 filter 含非标准档则保留，不含则不出现——不过滤 discovery 不在此处实现。
    ...
    async function applyProbeResults(results) {
      ...
      for (const result of Array.isArray(results) ? results : []) {
        ...
        const verified = {}
        // MAINT-014：标准档使用生成线值；非标准档（discovery 发现级）以等级名自身作为线值
        ...
        for (const level of working) {
          if (!GENERATED_LEVELS.includes(level)) verified[level] = level
        }
