...
/** 统一设置项：llm-reasoning { enabled, level, models, syncDefaultAgent, purposes, statsPublic }，写入 $DSH_HOME/settings.yaml。 */
...
export function apply(ctx) {
  ...
  if (webServer !== undefined) {
    ...
    // P0-2 实测按钮：发一个 1-token 请求验证某模型某等级实际可用。
    ctx.effect(() => webServer.register({
      kind: 'exact',
      ...
      handler: (req, res) => {
        ...
        req.on('end', async () => {
          ...
          let input
          try { input = JSON.parse(body || '{}') } catch (error) { input = {} }
          ...
          try {
            ...
            const ok = finish === undefined || finish.kind !== 'error'
            if (finish !== undefined && (finish.kind === 'error' || finish.kind === 'aborted')) {
              const level = rejectionLevelOf(finish.failure)
              if (level !== undefined) markRejected(provider, model, level)
            }
            ...
            res.end(JSON.stringify({
              ...
              finish: finish !== undefined && typeof finish.kind === 'string' ? finish.kind : 'unknown',
              blacklisted: effort !== undefined && isBlacklisted(provider, model, effort),
            }))
          } catch (error) {
            const level = rejectionLevelOf(error)
            if (level !== undefined) markRejected(provider, model, level)
            res.writeHead(200, { 'content-type': 'application/json; charset=utf-8' })
            res.end(JSON.stringify({ ok: false, provider, model, level: effort ?? null, error: error && error.message ? error.message : String(error) }))
          }
        })
      },
