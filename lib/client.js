  ...
  factory: (require) => {
    ...
    const LANG = {
      zh: {
        ...
        testingNote: '一键探测会对每个模型发 1-token 实测请求验证全部候选等级（网关实测拒绝自动入黑名单并持久化），结束后把可用等级固化进模型能力声明；导出可下载 CSV。',
        ...
        probeDone: (n, w) => `探测完成：${n} 个模型，固化 ${w} 处模型能力声明；实测拒绝的等级已入黑名单并持久化（重启后仍生效）。`,
        ...
        probeNote: '失败等级入黑名单后，后续请求自动跳过该档；可用的等级写回该模型能力声明（用户手写的声明不会被覆盖）。',
      },
      en: {
        ...
        testingNote: 'Probe-all sends a 1-token request per candidate level (gateway rejection auto-blacklists and persists), then writes working levels back to capability declarations; Export downloads CSV.',
        ...
        probeDone: (n, w) => `Probe complete: ${n} models, ${w} capability declarations fixed; rejected levels are blacklisted and persisted.`,
        ...
        probeNote: 'Rejected levels are blacklisted so later requests skip them; working levels are written back to the model capability declaration (hand-written declarations are never overwritten).',
      },
    }
    ...
    function StatsPanel(props) {
      ...
      useEffect(() => {
        let alive2 = true
        // 探测目标 = 全部可调用模型（router 包装路由跳过：能力与底层路由一致，避免重复实测烧请求）
      ...
      const probeAll = () => {
        ...
        ;(async () => {
          ...
          try {
            ...
            if (applied.error !== undefined) applyNote += '（' + applied.error + '）'
          } catch (error) {
      ...
      return el('div', null,
        ...
        el('div', { style: rowStyle },
          el('span', { style: labelStyle }, '实测探测'),
          el('button', {
            type: 'button',
            disabled: probing || probeTargets.length === 0,
            onClick: probeAll,
            style: { padding: '4px 10px', fontWeight: 600 },
          }, probing ? '探测中…' : '一键探测全部模型并固化配置'),
          el('button', { type: 'button', onClick: exportCsv, style: { padding: '4px 10px' } }, '导出 CSV'),
          el('span', { style: { fontSize: '12px', opacity: 0.65 } }, '（' + String(probeTargets.length) + ' 个模型 · 实测用 1-token 请求）'),
        ),
        probeProgress !== '' ? el('div', { style: statusStyle }, probeProgress) : null,
        probeSummary !== '' ? el('div', { style: { ...statusStyle, color: '#2e7d32' } }, probeSummary) : null,
        probeResults.length > 0
          ? el('div', null,
              el('div', { style: sectionTitle }, '探测结果'),
              el('table', { style: tableStyle },
                el('thead', null, el('tr', null,
                  el('th', { style: cellStyle }, '模型'),
                  el('th', { style: cellStyle }, '实测可用等级'),
                  el('th', { style: cellStyle }, '实测拒绝'),
                  el('th', { style: cellStyle }, '其他错误'),
                )),
                el('tbody', null, probeResults.map((r, i) => el('tr', { key: i },
                  ...
                  el('td', { style: cellStyle, color: (r.working ?? []).length > 0 ? '#2e7d32' : undefined },
                    (r.working ?? []).map(probeLevelLabel).join('/') || (r.error !== undefined ? '—' : '—')),
